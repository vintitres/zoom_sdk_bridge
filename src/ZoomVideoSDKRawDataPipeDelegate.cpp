...
std::vector<ZoomVideoSDKRawDataPipeDelegate *>
    ZoomVideoSDKRawDataPipeDelegate::list_;
...
ZoomVideoSDKRawDataPipeDelegate::ZoomVideoSDKRawDataPipeDelegate(
    IZoomVideoSDKUser *user, bool isShareScreen) {
  instance_id_ = instance_count++;
  user_ = user;
  // TODO
...
ZoomVideoSDKRawDataPipeDelegate::~ZoomVideoSDKRawDataPipeDelegate() {
  ...
  log(L"********** [%d] Finishing encoding, user: %s, %dx%d.\n", instance_id_,
      user_->getUserName(), in_width, in_height);
...
ZoomVideoSDKRawDataPipeDelegate::find_instance(IZoomVideoSDKUser *user) {
...
void ZoomVideoSDKRawDataPipeDelegate::stop_encoding_for(
    IZoomVideoSDKUser *user) {
  ZoomVideoSDKRawDataPipeDelegate *encoder =
      ZoomVideoSDKRawDataPipeDelegate::find_instance(user);
  if (encoder) {
...
void ZoomVideoSDKRawDataPipeDelegate::onRawDataFrameReceived(
  ...
  if ((sourceID != current_sourceID) &&
      (sourceID == 0 ||
       strlen(userID) >
           0) // to skip frames when sourceID comes in but userID is not ready,
              // otherwise create another sepreate file for this moment.
  ) {
...
void ZoomVideoSDKRawDataPipeDelegate::onRawDataStatusChanged(
    RawDataStatus status) {
  log(L"********** [%d] onRawDataStatusChanged, user: %s, %d.\n", instance_id_,
      user_->getUserName(), status);
  if (status == RawData_On) {
  } else {
  }
}
void ZoomVideoSDKRawDataPipeDelegate::onShareCursorDataReceived(
    ZoomVideoSDKShareCursorData info) {
  // Implement cursor data handling if needed, or leave empty if not using
  // cursor data
}
