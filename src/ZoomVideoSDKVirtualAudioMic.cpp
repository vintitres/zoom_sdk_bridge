...
void ZoomVideoSDKVirtualAudioMic::SendRawAudio(char *data,
  ...
  // See zoom_video_sdk_audio_send_rawdata_interface.h for raw audio data
  // information
  virtual_audio_sender_->Send(data, data_length, sample_rate);
}

void ZoomVideoSDKVirtualAudioMic::onMicInitialize(
    IZoomVideoSDKAudioSender *rawdata_sender) {
  printf("onMicInitialize\n");
  // Once the sender has been recieved from this callback, then Send can be
  // called
